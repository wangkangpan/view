...
function requestAnnouncements(element,pageNo,pageSize){
    $.post(
        user + "/comments/getAnnouncements",
        {
            pageNo: pageNo,
            pageSize: pageSize
        }
...
function requestAnnouncementsCount(){
    ...
    $.post(
        user + "/comments/getCountsAnnouncements"
    ).done(function (count) {
...
function requestCommentsCount(){
    ...
    $.ajaxSettings.async = false;
    $.post(
        user + "/comments/getCountsComments"
    ).done(function (count) {
        PageNum = Math.ceil(count / pageSize);
    }).fail(function () {
        toastr.error("获取公告数量失败");
...
function getAnnouncementById(id){
    ...
    $.ajaxSettings.async = false;
    $.post(
        user + "/comments/getAnnouncementById",
        {
            id: id
        }
    ).done(function (item) {
